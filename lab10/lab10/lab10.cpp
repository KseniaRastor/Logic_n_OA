...
void process(int n, int** G, int* dist){
	...
	int* center_t = (int*)malloc(n * sizeof(int*));										//центр тяжести
	int sum_dist = 0;
	for (int i = 1; i <= n; i++) {
		BFSD(i, n, G, dist);

		for (int i_sum_dist = 1; i_sum_dist <= n; i_sum_dist++) {
			sum_dist += dist[i_sum_dist];
		}
		center_t[i] = sum_dist;
	...
	int center = 1000;
	int center_index = 0;
	for (int i_4_center = 1; i_4_center <= n; i_4_center++) {
		if (center > center_t[i_4_center]) {
			center = center_t[i_4_center];
			center_index = i_4_center;
		}
	}

	if (center == 0) printf("Вершина изолирована\n");

	printf("Центр тяжести находится в вершине %d = %d\n", center_index, center);
